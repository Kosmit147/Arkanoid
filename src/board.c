...
{
    return (float)coord / (float)COORDINATE_SPACE * 2.0f - 1.0f;
}

void normalizeBlockCoordinates(float* normalizedPositions, Vec2ui position, unsigned int width, unsigned int height)
{
    float normalizedX1 = normalizeCoordinateui(position.x);
    float normalizedY1 = normalizeCoordinateui(position.y);
